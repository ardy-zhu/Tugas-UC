...
const GAME_SETTINGS = {
  // auto: random with AUTO_WIN_CHANCE, always_win: force wins when possible, always_lose: force losses when possible
  // lose2_win1_then_always_lose: lose 2 spins, win 1 spin, then lose forever (until reset)
  RESULT_MODE: "auto",
  AUTO_WIN_CHANCE: 0.35,
};

let scriptedResultState = {
  lossesBeforeSingleWin: 0,
  oneWinAlreadyGiven: false,
};
...
function resetGame() {
  ...
  numbersBet = [];
  previousNumbers = [];
  scriptedResultState.lossesBeforeSingleWin = 0;
  scriptedResultState.oneWinAlreadyGiven = false;
...
function getNumbersNotBet() {
  let notBet = [];

  for (let i = 0; i <= 36; i++) {
    if (!numbersBet.includes(i)) {
      notBet.push(i);
    }
  }
...
function getWinningSpinBySettings() {
  if (numbersBet.length === 0) {
    return Math.floor(Math.random() * 37);
  }

  if (GAME_SETTINGS.RESULT_MODE === "lose2_win1_then_always_lose") {
    let losingNumbers = getNumbersNotBet();
    let fallbackRandom = Math.floor(Math.random() * 37);

    if (scriptedResultState.oneWinAlreadyGiven) {
      return losingNumbers.length > 0
        ? getRandomFromArray(losingNumbers)
        : fallbackRandom;
    }

    if (scriptedResultState.lossesBeforeSingleWin < 2) {
      scriptedResultState.lossesBeforeSingleWin++;
      return losingNumbers.length > 0
        ? getRandomFromArray(losingNumbers)
        : fallbackRandom;
    }

    scriptedResultState.oneWinAlreadyGiven = true;
    return getRandomFromArray(numbersBet);
  }

  if (GAME_SETTINGS.RESULT_MODE === "always_win") {
    return getRandomFromArray(numbersBet);
  }

  if (GAME_SETTINGS.RESULT_MODE === "always_lose") {
    let losingNumbers = getNumbersNotBet();
    return losingNumbers.length > 0
      ? getRandomFromArray(losingNumbers)
  ...
  // Auto mode uses configured win chance.
  let shouldWin = Math.random() < GAME_SETTINGS.AUTO_WIN_CHANCE;
  if (shouldWin) {
    return getRandomFromArray(numbersBet);
  }
...
function setResultMode(mode) {
  let allowedModes = [
    ...
    "always_win",
    "always_lose",
    "lose2_win1_then_always_lose",
  ];
  if (!allowedModes.includes(mode)) return;

  GAME_SETTINGS.RESULT_MODE = mode;

  if (mode !== "lose2_win1_then_always_lose") {
    scriptedResultState.lossesBeforeSingleWin = 0;
    scriptedResultState.oneWinAlreadyGiven = false;
  }
}

window.setResultMode = setResultMode;

function updateBankAndBetDisplay() {
  document.getElementById("bankSpan").innerText =
    "" + bankValue.toLocaleString("en-GB") + "";
  document.getElementById("betSpan").innerText =
    "" + currentBet.toLocaleString("en-GB") + "";
...
function renderChipAmount(cell, amount) {
  ...
  chip.style.cssText = "";
  chip.setAttribute("class", "chip " + getChipColour(amount));
  chip.querySelector(".chipSpan").innerText = amount;
}

function ensureSpinButton() {
  if (container.querySelector(".spinBtn")) return;
...
function setBet(e, n, t, o) {
  ...
  renderChipAmount(e, wager);
}

function spin() {
  let winningSpin = getWinningSpinBySettings();
  ...
  for (let i = 0; i < bet.length; i++) {
    if (bet[i].numbers == n && bet[i].type == t) {
      if (bet[i].amt != 0) {
        ...
        bet[i].amt = bet[i].amt - removeAmount;
        bankValue = bankValue + removeAmount;
        currentBet = currentBet - removeAmount;
        updateBankAndBetDisplay();
        if (bet[i].amt == 0) {
